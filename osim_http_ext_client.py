...
class Client(object):
    ...
    """
    def __init__(self, p=0, visualize=False, rb='127.0.0.1'):
        port = 5000 + p
        self.remote_base = 'http://' + rb + ':' + str(port)
        self.session = requests.Session()
        self.session.headers.update({'Content-type': 'application/json'})
        self.env_create(visualize)
        # return ob
    ...
    def reset(self):
        return np.array(self.env_reset())

    def step(self, action, render=False):
        o, r, d, i = self.env_step(action.tolist(), render)
        o = np.array(o)
        return o, r, d, i
